import {
  SectionHeaderData,
  sectionHeaderStruct,
} from "./structs/section-header.struct";
...
export class TIM {
  ...
  constructor(arrayBuffer: ArrayBuffer, startOffset = 0) {
    ...
    if (this.hasCLUT) {
      ...
      pixelDataStart =
        this.header.nextOffset + this.clutHeader.sectionByteLength;
      ...
      for (let i = this.clutHeader.nextOffset; i < pixelDataStart; i += 2) {
        ...
        const clutPixel = PSXColor.FromSixteenBitValue(
    ...
    this.pixelDataHeader = sectionHeaderStruct.createObject<SectionHeaderData>(
      arrayBuffer,
      pixelDataStart,
      true,
    );
    ...
    const pixelDataSizeInBytes =
      this.pixelDataHeader.dataWidth * this.pixelDataHeader.dataHeight * 2;
    const pixelDataEnd = this.pixelDataHeader.nextOffset + pixelDataSizeInBytes;
    this.pixelData = new Uint16Array(
      arrayBuffer.slice(this.pixelDataHeader.nextOffset, pixelDataEnd),
    );
  ...
  get texturePage(): number {
    const xPage = Math.floor(
      this.pixelDataHeader.vramX / VRAM.TEXTURE_PAGE_NATIVE_WIDTH,
    );
    const yPage = Math.floor(
      this.pixelDataHeader.vramY / VRAM.TEXTURE_PAGE_NATIVE_HEIGHT,
    );
  ...
  createImageData(clutX?: number, clutY?: number): ImageData | undefined {
    ...
    if (this.hasCLUT && this.pixelMode === TIMPixelMode.FOUR_BIT_CLUT) {
      ...
      !this.hasCLUT &&
      this.pixelMode === TIMPixelMode.FIFTEEN_BIT_CLUT
    ) {
      return this.create16BitImageData();
    } else if (
      !this.hasCLUT &&
      this.pixelMode === TIMPixelMode.TWENTY_FOUR_BIT_CLUT
    ) {
  ...
  create4BitImageData(clut: PSXColor[]): ImageData {
    const imageData = new ImageData(
      this.pixelDataHeader.dataWidth * TIM.FourBitMultiplier,
      this.pixelDataHeader.dataHeight,
    );
    this.pixelData.forEach((pixelData, index) => {
      ...
      this.setImageDataPixel(
        imageData,
        imageDataIndex + 4,
        clut[secondColorIndex],
      );
      this.setImageDataPixel(
        imageData,
        imageDataIndex + 8,
        clut[thirdColorIndex],
      );
      this.setImageDataPixel(
        imageData,
        imageDataIndex + 12,
        clut[fourthColorIndex],
      );
    });
  ...
  create8BitImageData(clut: PSXColor[]): ImageData {
    const imageData = new ImageData(
      this.pixelDataHeader.dataWidth * TIM.EightBitMultiplier,
      this.pixelDataHeader.dataHeight,
    );
    this.pixelData.forEach((pixelData, index) => {
  ...
  create16BitImageData(): ImageData {
    const imageData = new ImageData(
      this.pixelDataHeader.dataWidth,
      this.pixelDataHeader.dataHeight,
    );
    this.pixelData.forEach((pixelData, index) => {
  ...
  create24BitImageData(): ImageData {
    const imageData = new ImageData(
      this.pixelDataHeader.dataWidth * TIM.TwentyFourBitMultiplier,
      this.pixelDataHeader.dataHeight,
    );
  ...
  private checkCLUTAccuracy() {
    if (
      this.clutHeader.sectionByteLength >
      VRAM.VRAM_BYTE_WIDTH * VRAM.VRAM_BYTE_HEIGHT
    ) {
      throw new Error(
        `Invalid CLUT Section Byte Size, ${this.clutHeader.sectionByteLength}`,
      );
    }

    if (this.clutHeader.vramY > 512 || this.clutHeader.vramX > 1024) {
      throw new Error(
        `Invalid CLUT Position, X: ${this.clutHeader.vramX}, Y: ${this.clutHeader.vramY}`,
      );
    }

    if (
      this.clutHeader.dataWidth > VRAM.VRAM_BYTE_HEIGHT ||
      this.clutHeader.dataHeight > VRAM.VRAM_BYTE_HEIGHT
    ) {
      throw new Error(
        `Invalid CLUT Size, Width: ${this.clutHeader.dataWidth}, Height: ${this.clutHeader.dataHeight}`,
      );
    }
  }
  ...
  private checkPixelDataAccurace() {
    if ((this.bitsPerPixel === 4 || this.bitsPerPixel === 8) && !this.hasCLUT) {
      throw new Error(
        `Invalid structure. No palette found for ${this.bitsPerPixel} image.`,
      );
    }

    if (
      this.pixelDataHeader.sectionByteLength >
      VRAM.VRAM_BYTE_WIDTH * VRAM.VRAM_BYTE_HEIGHT
    ) {
      throw new Error(
        `Invalid Pixel Data Section Byte Size, ${this.pixelDataHeader.sectionByteLength}`,
      );
    }

    if (
      this.pixelDataHeader.vramX > VRAM.VRAM_NATIVE_WIDTH ||
      this.pixelDataHeader.vramY > VRAM.VRAM_BYTE_HEIGHT
    ) {
      throw new Error(
        `Invalid Pixel Data Position, X: ${this.pixelDataHeader.vramX}, Y: ${this.pixelDataHeader.vramY}`,
      );
    }

    if (
      this.pixelDataHeader.dataWidth > VRAM.VRAM_BYTE_WIDTH ||
      this.pixelDataHeader.dataHeight > VRAM.VRAM_BYTE_WIDTH
    ) {
      throw new Error(
        `Invalid Pixel Data Size, Width: ${this.pixelDataHeader.dataWidth}, Height: ${this.pixelDataHeader.dataHeight}`,
      );
    }
  }
}
